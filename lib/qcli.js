...
// Transcribe audio file using extract-speech command
function transcribeAudio(filePath) {
  try {
    const transcription = execSync(`extract-speech "${filePath}"`, {
      encoding: 'utf8',
...
${convertedContent}`;
    ...
    // If ACL would be exceeded, return null to prevent response from being sent
    if (wouldExceedACL) {
      if (debug) {
        log(`Blocking response due to ACL limit (${currentACL} >= ${maxACL})`);
      }
      return null;
    }
  ...
  try {
    ...
    const messageData = {
      ...
      messages: messages.map(msg => {
        ...
        return {
          ...
          acl: msgACL,
          informationalOnly: wouldExceedACL
        };
      })
    };
    ...
    try {
      if (existsSync(outputFile)) {
        ...
        // Filter out responses to informationalOnly messages
        ...
        result = responsesArray.filter(response => {
