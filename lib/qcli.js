...
async function executeQCLI(query, agentName, isBatch = false) {
  activeProcesses++;
...
export async function processRealtimeMessage(message, channel, agentName, debugJson = false, noDiscord = false) {
...
${convertedContent}`;
    ...
    const response = await executeQCLI(query, agentName);
...
export async function processBatchedMessages(messages, channel, agentName, debugJson = false, noDiscord = false) {
  // Circuit breaker - stop if failing repeatedly
  if (consecutiveFailures >= MAX_FAILURES) {
    log(`Circuit breaker: ${consecutiveFailures} failures, not trying`);
    return [];
  ...
  try {
    ...
    await executeQCLI(query, agentName, true);
