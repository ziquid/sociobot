...
async function executeQCLI(query, agentName, authorUsername, channel, messageTimestamp, isBatch = false, debug = false) {
  activeProcesses++;
...
${convertedContent}`;
    ...
    const response = await executeQCLI(query, agentName, message.author.username, channel, message.createdAt.toLocaleString(), false, debug);
